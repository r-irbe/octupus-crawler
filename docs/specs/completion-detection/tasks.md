...
- [x] **T-COORD-011**: Implement idempotent seeding via frontier dedup → REQ-DIST-020

## Phase 4: State-Store Connection

- [x] **T-COORD-012**: Implement connection string parsing (host, port, password, ACL, TLS) → REQ-DIST-021
- [x] **T-COORD-013**: Attach error handlers to all event-emitting components → REQ-DIST-022
...
- [x] **T-COORD-016**: Implement failover: standby acquires lease, re-derives state → REQ-DIST-024, REQ-DIST-025
