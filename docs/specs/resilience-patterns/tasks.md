...
- [ ] **T-RES-009**: Implement per-domain token bucket rate limiter with configurable burst and refill — REQ-RES-010
- [ ] **T-RES-010**: Implement Redis sliding window API rate limiter — REQ-RES-011
- [ ] **T-RES-011**: Implement bulkhead policy with per-domain concurrency limiting (default 2) — REQ-RES-012, REQ-RES-013

## Phase 4: Fallback & DLQ

- [ ] **T-RES-012**: Implement fallback strategy serving cached stale data on circuit open — REQ-RES-014
- [ ] **T-RES-013**: Add degraded mode metrics and structured logging — REQ-RES-015
...
- [ ] **T-RES-016**: Create `createFetchPolicy(domain)` that composes full 7-layer stack — REQ-RES-018, REQ-RES-019
...
- [ ] **T-RES-020**: Wire Zod-validated configuration for all resilience parameters — REQ-RES-003, REQ-RES-008
